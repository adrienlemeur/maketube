...

{
	...
	make_option(c("--deletion_count"), type='numeric', default=as.numeric(10),
	            help="number of large deletions"),
...
for(SV_set in 1:opt$structural_variants){
	...
	dir.create(SV_folder_path, showWarnings = FALSE)
	{
		...
		hist(rgamma(10000, shape = 2, scale = 3000), breaks = 100)
		qgamma(c(0.005, 0.999), shape = 2, scale = 3000)
		pgamma(10000, shape = 2, scale = 3000, lower.tail = T)
