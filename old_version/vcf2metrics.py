...
#add one to every variant position. Deprecated.
def add_one(my_dict):
	new_dict = {}
...
#import a annotation bed file into a dict object.
#key = 4th column (annotation)
#value = start / stop
def bed2dict(files):
	bed_dict = {}
...
#print the trace for backtracking, for debugging
#the trace can be manually parsed to check for discrepancies in backtracking
#for each variant, print its position prior to backtrack, and its position after backtrack
#also prints the positions of strutuctural variants
#very similar to compareVCF
	...
	for vcf_dict in ref_dict_list:
		for variant in vcf_dict:
			if vcf_dict[variant]['CLASS'] in variant_class:
				...
				my_merge_dictionary[variant_new_key] = {}
				my_merge_dictionary[variant_new_key]['START'] = vcf_dict[variant]['START']
				my_merge_dictionary[variant_new_key]['END'] = vcf_dict[variant]['END']
				my_merge_dictionary[variant_new_key]['PREVIOUS_START'] = "REF"
				my_merge_dictionary[variant_new_key]['LENGTH'] = vcf_dict[variant]['END'] - vcf_dict[variant]['START']
				my_merge_dictionary[variant_new_key]['CLASS'] = vcf_dict[variant]['CLASS']
				my_merge_dictionary[variant_new_key]['REF'] = vcf_dict[variant]['REF']
				my_merge_dictionary[variant_new_key]['ALT'] = vcf_dict[variant]['ALT']
	...
	for vcf_dict in vcf_dict_list:
		for variant in vcf_dict:
			if vcf_dict[variant]['CLASS'] in variant_class:
				...
				my_merge_dictionary[variant] = {}
				my_merge_dictionary[variant]['START'] = vcf_dict[variant]['START']
				my_merge_dictionary[variant]['END'] = vcf_dict[variant]['END']
				if 'PREVIOUS_START' in vcf_dict[variant]:
					my_merge_dictionary[variant]['PREVIOUS_START'] = vcf_dict[variant]['PREVIOUS_START']
				else:
					my_merge_dictionary[variant]['PREVIOUS_START'] = vcf_dict[variant]['PREVIOUS_START']
				my_merge_dictionary[variant]['LENGTH'] = vcf_dict[variant]['END'] - vcf_dict[variant]['START']
				my_merge_dictionary[variant]['CLASS'] = vcf_dict[variant]['CLASS']
				my_merge_dictionary[variant]['REF'] = vcf_dict[variant]['REF']
				my_merge_dictionary[variant]['ALT'] = vcf_dict[variant]['ALT']

	for backtrack_dict in backtrack_list:
		for region in backtrack_dict:
			if backtrack_dict[region]['class'] == "insertion":
				my_merge_dictionary[region] = {}
				my_merge_dictionary[region]['PREVIOUS_START'] = "NA"
				my_merge_dictionary[region]['REF'] = "NA"
				my_merge_dictionary[region]['ALT'] = "NA"

				my_merge_dictionary[region]['START'] = backtrack_dict[region]['ins_start']
				my_merge_dictionary[region]['END'] = backtrack_dict[region]['ins_stop']
				my_merge_dictionary[region]['LENGTH'] = my_merge_dictionary[region]['END'] - my_merge_dictionary[region]['START']
				my_merge_dictionary[region]['CLASS'] = "insertion"
			...
			if backtrack_dict[region]['class'] == "deletion":
				...
				my_merge_dictionary[region]['START'] = backtrack_dict[region]['start']
				my_merge_dictionary[region]['END'] = backtrack_dict[region]['stop']
				my_merge_dictionary[region]['LENGTH'] = my_merge_dictionary[region]['START'] - my_merge_dictionary[region]['END']
				my_merge_dictionary[region]['CLASS'] = "deletion"

			if backtrack_dict[region]['class'] == "transposon":

				initial_pos_key = region+"_initial_position"
				my_merge_dictionary[initial_pos_key] = {}
				my_merge_dictionary[initial_pos_key]['PREVIOUS_START'] = "NA"
				my_merge_dictionary[initial_pos_key]['REF'] = "NA"
				my_merge_dictionary[initial_pos_key]['ALT'] = "NA"

				my_merge_dictionary[initial_pos_key]['START'] = backtrack_dict[region]['start']
				my_merge_dictionary[initial_pos_key]['END'] = backtrack_dict[region]['stop']
				my_merge_dictionary[initial_pos_key]['LENGTH'] = my_merge_dictionary[initial_pos_key]['START'] - my_merge_dictionary[initial_pos_key]['END']
				my_merge_dictionary[initial_pos_key]['CLASS'] = "transposon_initial_position"

				final_pos_key = region+"_post_jump_position"
				my_merge_dictionary[final_pos_key] = {}
				my_merge_dictionary[final_pos_key]['PREVIOUS_START'] = "NA"
				my_merge_dictionary[final_pos_key]['REF'] = "NA"
				my_merge_dictionary[final_pos_key]['ALT'] = "NA"
				
				my_merge_dictionary[final_pos_key]['START'] = backtrack_dict[region]['ins_start']
				my_merge_dictionary[final_pos_key]['END'] = backtrack_dict[region]['ins_stop']
				my_merge_dictionary[final_pos_key]['LENGTH'] = my_merge_dictionary[initial_pos_key]['END'] - my_merge_dictionary[initial_pos_key]['START']
				my_merge_dictionary[final_pos_key]['CLASS'] = "transposon_post_jump"


	sort_my_merge_dictionary = dict(sorted(my_merge_dictionary.items(),key=lambda x: int(x[1]['START'])))

	for key in sort_my_merge_dictionary:
		try:
			print(sort_my_merge_dictionary[key]['START'], sort_my_merge_dictionary[key]['END'], sort_my_merge_dictionary[key]['PREVIOUS_START'], sort_my_merge_dictionary[key]['CLASS'], sort_my_merge_dictionary[key]['LENGTH'], sort_my_merge_dictionary[key]['REF'], sort_my_merge_dictionary[key]['ALT'], sep = "\t")
		except:
			print("")
...
if args.bed is not None:
	regions_dictionary = bed2dict(args.bed)
else:
	regions_dictionary = None

if(args.backtrack):
	backtrack_dict = read_backtrack(args.backtrack[0])

reference_dictionary = sort_dict(vcf2dict(VCF(args.reference[0]), 0))

comparison_dict = {}

for sample_vcf in args.i:
	sample_dictionary = sort_dict(vcf2dict(VCF(sample_vcf), 0))

	if(args.backtrack):
		backtracked_sample_vcf_dictionary = sort_dict(backtrack(sample_dictionary, backtrack_dict))

		comparison_dict = compareVCF(reference_dictionary, backtracked_sample_vcf_dictionary, regions_dictionary)
	else:
		comparison_dict = compareVCF(reference_dictionary, sample_dictionary, regions_dictionary)

	if(args.trace):
		vcfbed2dict4table(vcf_dict_list = [ backtracked_sample_vcf_dictionary ], ref_dict_list = [ reference_dictionary ], backtrack_list = [ backtrack_dict ], variant_class = 'snp')
