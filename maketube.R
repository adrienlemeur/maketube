...

{
  ...
  option_list = list(
    make_option(c("--reference"), type="character", default=NULL, 
                help="reference sequence to be used as a template for evolved genomes (fasta)", metavar="character"),
    ...
    make_option(c("--nonhomoseq_pool"), type="character", default=NULL,
                help="a list of kmers to pick from to create ancestral-like sequence (fasta without header)", metavar="character"),
    make_option(c("--duplication_region_size"), type="numeric", default=150000, 
                help="size of the duplicated region (numeric)", metavar="character"),
    make_option(c("--haplotype_count"), type='numeric', default=10,
                help="number of haplotype to compute (>=1) (numeric)"),
    make_option(c("--pop_count"), type='numeric', default=1,
                help="number of haplotype to compute (>=1) (numeric)"),
    make_option(c("--structural_variants"), type='numeric', default=2,
                help="number of structural variants (numberic)"),
    make_option(c("--deletion_count"), type='numeric', default=3,
                help="number of deletion region to remove (numeric)"),
    make_option(c("--unmuted"), action = "store_true", default = TRUE,
                help="number of haplotype to compute (>=1) (numeric)"),
    make_option(c("-p", "--prefix"), type='character', default="my_run",
                help="artificial genome name prefix (character)"),
    make_option(c("--mutation_rate"), type='character', default=as.numeric(1.23e-7),
                help="the genome wide mutation rate (numeric)"),
    make_option(c("--effective_pop"), type='character', default=as.numeric(2000),
                help="the effective population size (numeric)"),
    make_option(c("--TCAGI"), type = "character", default = "0.172,0.329,0.172,0.329,0",
                help="the rate of the different nucleotide (TCAG) + invariants (I) (numeric, comma separated list)"),
    make_option(c("--ABCDEF"), type = "character", default = "0.65,0.05,0.21,0.27,0.02,0.65",
                help="the parameters of the GTR model  (numeric, comma separated list)"),
    make_option(c("--indel_scaling_factor"), type = "character", default=as.numeric(0.125),
                help="indel/snp scaling factor (numeric)"),
    make_option(c("--slope"), type='character', default="50,300",
                help="Size of the slope between two structural variants (numeric)"),
    make_option(c("--threads"), type='numeric', default=4,
                help="Number of threads used for fastq generation (numeric)"),
